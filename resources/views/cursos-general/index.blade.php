        ...
        <div class="row justify-content-center">
            <div class="col-lg-12">
                <div class="wrap-base fix-w mt-5">
                    <h6 class="titulo-seccion"><i class="cil-check-circle"></i> Listado de promocionales</h6>
                    <h6 class="info-sec-tit mt-3">Detalle con información de los promocionales</h6>
                                    ...
                                    <tbody>
                                        @foreach($cursos as $curso)
                                            <tr>
                                                ...
                                                <td>{{$curso->code}}</td>
                                                <td>
                                                    <a href="{{ URL::action('CursoGeneralController@edit', $curso->id) }}" class="btn btn-info text-white btn-editar">
                                                        <i class="cil-pencil"></i>
                                                    </a>
                                                    <a class="btn btn-danger text-white btn-eliminar" onclick="eliminarSweet({{$curso->id}})">
                                                        <i class="cil-trash"></i>
                                                    </a>
                                                    <form id="{{$curso->id}}" action="{{ URL::action('CursoGeneralController@destroy', $curso->id) }}" method="POST" style="display: none;">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </td>
