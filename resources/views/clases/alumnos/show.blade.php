        ...
        <div class="row justify-content-center">
            <div class="col-lg-12">
                <div class="wrap-base fix-w mt-5">
                    ...
                    <h6 class="info-sec-tit mt-3">{{$curso->name}}</h6>
                    <p class="info-seccion-aux">{{$curso->description}}</p>
                    <form method="POST" action="{{  URL::action('ClaseController@changeClass', encrypt($curso->id)) }}" class="mt-4 aprobar-form">
                    @csrf
                        <div class="row">
                            <div class="col-lg-5">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control form-select" name="lesson_id" id="clasesteo" aria-label="Default select example" required>
                                        ...
                                        @foreach($curso->clases as $clas)
                                            <option value="{{$clas->id}}" {{ ( $clas->id == $clase->id) ? 'selected' : '' }}>{{$clas->name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <iframe width="560" height="315" src="{{$clase->link}}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                            ...
                            <div class="row mt-3">
                                ...
                                <div class="col-lg-9 boc-com">
                                    <!--<a href="" class="enlace-com" data-bs-toggle="modal" data-bs-target="#comentario">-->
                                        <p class="titulo-com">{{$comentario->title}}</p>
                                        <p class="text-com">
                                            {{$comentario->comment}}
                                        </p>
                                        <span class="autor-com">{{$comentario->usuario->name}} <span class="datos-com"> {{$comentario->created_at}}</span></span>
                                    <!--</a>-->
                                </div>
                                <!--<div class="col-lg-2 num-box">
                                    <h4 class="num-com">#{{$comentario->id}}</h4>
                                </div>-->
                            </div>
                        @endif
                    @endforeach
...
<div class="modal fade" id="nuevocomentario" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="container">
            <form method="POST" action="{{  URL::action('ClaseController@storeComment', $clase->id) }}">
            @csrf
                <div class="mb-3">
                    <label for="titulo" class="form-label">Titulo del comentario</label>
                    <input type="text" class="form-control" name="title" id="titulo" placeholder="" aria-describedby="tituloHelp" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea id="textarea" rows="3" class="form-control" name="comment" id="comentarioBody" placeholder="Escribe tu comentario aqui..." aria-describedby="comentarioBodyHelp" required></textarea>
